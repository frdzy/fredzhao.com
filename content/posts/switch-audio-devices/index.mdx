...
currently paried to a phone), then it's possible that the Automator script will
fail. An alternative to the Quick Action is to run an AppleScript Application.
See more on configuring that in [this article](https://howchoo.com/macos/connect-airpods-with-keyboard-shortcut-in-macos).
